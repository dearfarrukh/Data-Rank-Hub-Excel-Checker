...
# Fingerprint the actual uploaded file contents so an edited Excel file
# is always treated as a new input, even when its filename and size are similar.
...
if not audit_df.empty:
    ...
    current_problem_entities = set(
        country_audit["Entity"].astype(str).tolist()
    )

    st.session_state["_completed_countries"] = [
        c for c in st.session_state["_completed_countries"]
        if c not in current_problem_entities
    ]
    ...
    def _sync_entity_decisions(entity_name, current_findings):
        valid_ids = {
            _problem_id(entity_name, finding)
            for _, finding in current_findings.iterrows()
        }

        decisions = st.session_state["_country_review_decisions"]
        prefix = str(entity_name) + "||"

        stale_keys = [
            key for key in list(decisions.keys())
            if str(key).startswith(prefix) and key not in valid_ids
        ]
    ...
    def _entity_review_status(entity_findings):
        ...
        # Keep entity labels consistent with the top summary:
        # only true Error-severity findings are MUST FIX.
    ...
    for country_name in priority_remaining:
        ...
        _sync_entity_decisions(country_name, findings)
        ...
        with st.expander(
            ...
            st.caption(
                "All currently detected problems for this entity are listed below. "
                "After any fix, this same dropdown is recalculated."
            )
            ...
            if pending == 0:
                st.success(
                    "All current problems for this entity have been reviewed."
                )
                if st.button(
                ...
                st.caption(
                    f"{pending} current problem(s) remaining. "
                    "After a fix, the entity is rechecked and this same list updates."
                )
                ...
                with st.expander(
                    ...
                    if blank_row is not None:
                        ...
                        if df_index in work.index:
                            preview_cols = [entity_column] + list(period_columns[:5])
                            preview_cols = [
