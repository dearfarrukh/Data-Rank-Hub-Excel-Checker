...
st.header("4. Review Countries")

# Country-first review state
...
if not audit_df.empty:
    ...
    # Priority = any country/entity that entered selected Top N at least once.
    priority_countries = []
    other_countries = []
    ...
    if priority_countries:
        st.info(
            f"Countries/entities that entered Top {int(important_rank)}: "
    ...
    for country_name in priority_remaining:
        ...
        with st.expander(
            ...
            if pending == 0:
                if st.button(
                    f"COUNTRY COMPLETE — {country_name}",
