...
if not audit_df.empty:
    ...
    def _entity_review_status(entity_findings):
        ...
        if "Error" in severities or categories.intersection(must_fix_categories):
            return "🔴 MUST FIX"
        easy_review_categories = {
            "Internal Gap", "Suspicious Jump",
            "Repeated Consecutive Value", "Zero Run"
        }
        if categories.intersection(easy_review_categories):
            return "🟡 EASY FIX / REVIEW"
        return "🟡 REVIEW"
                ...
                if decision == "Pending":
                    if plan is not None:
                        ...
                        if c1.button(
                            ...
                            corrected = st.session_state["_simple_checker_df"].copy()
                            corrected.loc[jump_plan["row_index"], jump_plan["column"]] = jump_plan["proposed"]
                            st.session_state["_simple_checker_df"] = corrected
                            st.session_state["_country_review_decisions"][pid] = "Fixed"
                            st.session_state["_simple_checker_log"].append({
                                "Entity": country_name, "Problem": category,
                                "Period": period_text,
                        ...
                        c1.button(
                            ...
                            disabled=True,
                            help="The app cannot safely calculate a replacement for this problem.",
                            use_container_width=True
                        )
